...
/*   Updated: 2020/12/11 13:45:00 by olidon           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
Form::Form(std::string const &name, int const required_sign, int const required_exec, std::string const &target) : _name(name), _required_sign(required_sign), _required_exec(required_exec), _target(target)
{
	_signed = false;
...
void				Form::execute(Bureaucrat const &executor) const
{
	...
	else if (_required_exec < executor.getGrade())
		throw Form::GradeTooLowException();
	else
		SpecificFormExec(_target);
}
